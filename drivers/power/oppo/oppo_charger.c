...
/*Jun.Wei@RM.BSP.CHG.Basic, 2019/05/09, work around for bq24190 CDP testing*/
...
/*vfloat_sw_limit*/
		...
		charger_xlog_printk(CHG_LOG_CRTI, "ff1_normal_fastchg_ma = %d,ffc2_temp_warm_decidegc = %d,ffc2_temp_high_decidegc = %d,ffc2_normal_fastchg_ma = %d,chip->limits.ffc2_warm_fastchg_ma = %d,ffc2_exit_step_ma = %d,ffc_normal_vfloat_sw_limit = %d,ffc2_normal_vfloat_sw_limit = %d,ffc2_temp_normal_vfloat_mv = %d,ffc_normal_vfloat_over_sw_limit = %d",
			chip->limits.ff1_normal_fastchg_ma,chip->limits.ffc2_temp_warm_decidegc,chip->limits.ffc2_temp_high_decidegc,chip->limits.ffc2_normal_fastchg_ma,chip->limits.ffc2_warm_fastchg_ma,chip->limits.ffc2_exit_step_ma,chip->limits.ffc1_normal_vfloat_sw_limit,chip->limits.ffc2_normal_vfloat_sw_limit,chip->limits.ffc_temp_normal_vfloat_mv,chip->limits.ffc_normal_vfloat_over_sw_limit);
...
static bool oppo_chg_check_ffc_status(struct oppo_chg_chip *chip)
{
	...
	if (chip->fastchg_to_ffc == true) {
		if (chip->fastchg_ffc_status == 1) {
			if (chip->ffc_temp_status == FFC_TEMP_STATUS__HIGH
...
/* wenbin.liu@SW.Bsp.Driver, 2016/02/29  Add for log tag*/
        ...
        chip->otg_switch, chip->mmi_chg, chip->boot_reason, chip->boot_mode, chip->chargerid_volt, chip->chargerid_volt_got, chip->usb_enum_type);
